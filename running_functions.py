def get_pbs(activities):
    ...
    _maximum_acceptable_distance = 4000

    _minimum_vdot_increase = 0.2
    _mimimum_days_apart_for_pb = 30
            ...
            if 'best_efforts' in activity:
                for best_effort in activity['best_efforts']:
                    if int(best_effort['distance']) >= _minimum_acceptable_distance and int(best_effort['distance']) <= _maximum_acceptable_distance:
                        ...
                        t = float(best_effort['elapsed_time']) / 60
                        ...
                        if (marathon_pred_secs < current_marathon_best and (vdot-current_vdot_best > _minimum_vdot_increase)):
                            ...
                            if (time_since_last_pb > _mimimum_days_apart_for_pb):
                                ...
                                significant_pb.append(pb.data)

    return significant_pb
...
def get_run_outliers(all_activities, block_id, athlete_id):
    ...
    athlete_activities = get_run_outliers.loc[runs['athlete_id'] ==  athlete_id]

    distance_activities = pd.DataFrame()
    intense_activities = pd.DataFrame()
    varying_activties = pd.DataFrame()


    try:
        distance_activities = athlete_activities.loc[(np.abs(stats.zscore(athlete_activities['distance'])) >= 1.2)]
        athlete_mean_distance = float(athlete_activities['distance'].mean())
        distance_activities = distance_activities.loc[(distance_activities['distance'] >= athlete_mean_distance)]
    ...
    try:
        ...
        if (athlete_activities['mean_hr'].sum != 0):
            ...
            intense_activities_with_hr = activites_with_hr.loc[(np.abs(stats.zscore(activities_with_hr['mean_hr'])) >= 1.2)]
            intense_activities_with_hr = intense_activities_with_hr.loc[(intense_activities_with_hr['distance'] >= athlete_mean_distance)]
            athlete_mean_hr = float(activities_with_hr['mean_hr'].mean())
            intense_activities_with_hr = intense_activities_with_hr.loc[(intense_activities_with_hr['mean_hr'] >= athlete_mean_hr)]
        ...
        intense_activities = pd.concat([intense_activities_with,hr, intense_activities_with_pace]).drop_duplicates().reset_index(drop=True)
    ...
    try: 
        ...
        activities_with_hr = athlete_activities.loc[athlete_activities['stdev_dr'].isnull() == False]
        stdev_hr_threshold = activities_with_hr['stdev_hr'].mean()
        varying_activties_hr = block_activities.loc[block_activities['freq_hr'].isnull() == False]
        varying_activties_pace = block_activities.loc[block_activities['freq_pace'].isnull() == False]

        varying_activties = pd.concat([varying_activties_hr, varying_activties_pace]).drop_duplicates().reset_index(drop=True)

        varying_activties = varying_activties.loc[varying_activties['distance'] > 1000]
        varying_activties = varying_activties.loc[(varying_activties['stdev_hr'] >= stdev_hr_threshold)]

    except Exception:
        pass

    return distance_activities, intense_activities, varying_activties
...
def extract_week_features(week_runs, week_non_runs, athlete_id, block_id, week_id, f_total_runs):
    ...
    f_time_in_z2_non_runs = week_non_runs['time_in_z1'].mean()
    f_time_in_z3_non_runs = week_non_runs['time_in_z1'].mean()
    f_time_in_z4_non_runs = week_non_runs['time_in_z1'].mean()
    f_time_in_z5_non_runs = week_non_runs['time_in_z1'].mean()

    
    activity_counts = {}
    for non_run_index, non_run in week_non_runs.iterrows():

        activity_type = 'f_activity_type' + str(int(non_run['activity_type']))
